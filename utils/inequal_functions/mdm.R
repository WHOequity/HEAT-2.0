...
mdm <- function(dat, bs=TRUE, bsiter = 200){
    ...
    if(bs){
      ...
      inequal.boot.vals <- sapply(1:n, function(x){
        ...
        if ((scaleval == 100 & all(estrand >= 0) & all(estrand <= 1)) | (scaleval != 100 & all(estrand >= 0))) {
          ...
          scaleval* sum(popsh * abs(estrand - sum(popsh * estrand)))
        }
      })
  ...
  return(list(inequal.mdm=inequal.mdm, se.mdm.boot=se.boot,  se.mdm.formula=NA, 
              se.lowerci.mdm = boot.lcl2, se.upperci.mdm = boot.ucl2))  # return a list of the inequality measure and the standard error 
